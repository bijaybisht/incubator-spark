...
import org.apache.spark.streaming.{Time, DStream, ProratedEventDStream, BinAlignedWindowDStream, PulsatingWindowDStream}

class BinStreamer[T: ClassManifest](@transient ds: DStream[T], getStartTime: (T) => Time, getEndTime: (T) => Time) extends Serializable {
